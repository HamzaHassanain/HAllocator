...
/**
 * @test Allocating entire block size works, prevents further allocations, and allows reallocation
 * after deallocation
 */
...
TEST(HallocBlockTest, SMALL_MultipleAllocations) {
    ...
    for (int i = vals.size() - 1; i >= 0; i--) {
        EXPECT_EQ(
            get_actual_value(((MemoryNode*)((unsigned char*)mem[i] - MEMORY_NODE_SIZE))->value),
            vals[i]);
    }
}

/**
 * @test Deallocations create merged free blocks enabling larger allocations (coalescing
 * verification)
 */
...
/**
 * @test 50K random allocations with 60% deallocation followed by 10K reallocations tests memory
 * reuse
 */
...
/**
 * @test Alternating allocation/deallocation patterns verify coalescing under heavy fragmentation
 * (10K+ allocs)
 */
...
/**
 * @test Exponentially increasing sizes (1KB to GB) with random deallocation stresses RB-tree
 * balancing (100K+ ops)
 */
TEST(HallocBlockTest, STRESS_WorstCaseRBTreeDepth) {
    ...
    for (std::size_t i = 0; i < 25; i++) {
        std::size_t size =
            base_size * (1ULL << i);  // 1KB, 2KB, 4KB, 8KB, ..., up to 16GB (but limited by block)
